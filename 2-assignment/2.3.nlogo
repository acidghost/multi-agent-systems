...
to go
  ...
  execute-actions
  tick
  if any? turtles with [xcor = max-pxcor and ycor = max-pycor] and all? patches [pcolor = black] [
    ask turtles [set color blue ] stop
  ]
end
...
; --- Setup patches ---
to setup-patches
  let dirt_cells (dirt_pct / 100) * ( count patches )
  let obstacle_cells (obstacle_pct / 100) * ( count patches)
  ; In this method you may create the environment (patches), using colors to define dirty and cleaned cells.
  ask patches [set pcolor white ]
  ask n-of dirt_cells patches [ set pcolor gray ]
  ask n-of obstacle_cells patches [ set pcolor black ]
...
to setup-turtles
  ...
  create-turtles 1 [ setxy min-pxcor min-pycor facexy min-pxcor min-pycor + 1 ]
...
to move-robot
  ask turtles [
    if-else pcolor = gray [
      ...
      [ if-else (random 1) = 1 [left 90][right 90] ]

      [ if-else  [pcolor] of patch-ahead 1 = black

        [ if-else (random 1) = 1 [left 90][right 90] ]

        [ if-else (random 100) < 80
          [ forward 1]
          [ if-else (random 1) = 1 [left 90][right 90] ]
        ]
      ]
