...
vacuums-own [beliefs desire intention dirt_in_bag battery]
patches-own [dirt_value]
...
to setup-patches
  ...
  ask n-of total_dirty patches [ set pcolor gray set dirt_value random 5 + 1 set plabel dirt_value ] ; set amount of dirt per dirty patch
end
...
to update-desires
  ...
  ask vacuums [
    if-else length beliefs = 0 or battery = 0 [
